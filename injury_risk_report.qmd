...
\vspace{0.7cm}
{\Large Hochschule Luzern (HSLU) \par}
\vspace{0.35cm}
{\large W.DBM02\_Database Management for Data Scientists \par}
\vspace{0.4cm}
{\large Team AAM \par}
...
\vspace{0.2cm}
Alina Caspar (alina.caspar@hslu.ch) \\
\vspace{0.2cm}
...
Soft-tissue injuries in professional football cost clubs an estimated EUR 500M per season in lost wages and medical costs, with hamstring strains alone accounting for more than 15% of all time-loss injuries. Sports-science literature (Gabbett, 2016; Hulin et al., 2016) shows that a disproportionate spike in acute training load relative to chronically accumulated fitness is a leading predictor of non-contact injuries. Yet many clubs still rely on subjective session-RPE or on proprietary wearable dashboards that coaches cannot easily inspect.

The primary objective of this project is to demonstrate core database management concepts. Using football injury prevention as an illustrative context, we design a relational data model, implement it in MySQL, and integrate three independent data sources using an ELT approach. SQL is then used to derive the Acute-to-Chronic Workload Ratio (ACWR), our injury risk key figure, and apply a published decision rule in an interactive Metabase dashboard.

Following the OECD Data Value Cycle from the lecture, raw data is transformed into structured information and decision-relevant metrics. Workload telemetry, injury event records, and player features are combined at the player-day level and shown in an interactive Metabase dashboard. The result is a dashboard where a coach can filter players, inspect risk bands, and see how the decision rule was calculated.
...
3.  **Player features**: age, position, sport type, and prior injury history
...
The three datasets are linked via a composite strategy: the multimodal dataset supplies the workload time-series per athlete, the European dataset provides named player and injury event data, and the university dataset contributes a static benchmark multiplier. All three are connected at aggregate level through the conformed dimensions `dim_age_group` and `dim_position_group`, which avoids claiming row-level identity across independent datasets.
...
An IRS substantially above 1 indicates the athlete is accumulating load faster than their long-term conditioning supports. This is the "sweet spot" violation described in the injury-risk literature. Because the multimodal dataset contains no real calendar dates, session order (derived from `session_id`) is used as the time axis; a synthetic date starting 2024-01-01 is assigned per athlete for `dim_date` compatibility.
...
-   **prior\_injury\_multiplier** (S3): $1.0 + 0.3 \times \text{avg\_previous\_injury\_count}$ per age group, capturing historical injury burden (Fuller et al., 2006)
-   **position\_age\_factor** (S2): ratio of age-position injury rate to overall average injury rate from the European dataset, capturing benchmark-level positional and age-related risk exposure (Ekstrand et al., 2011)

The base adjusted view joins factors at `age_group_id` level, the finest grain at which all three sources can be compared without a shared player identifier. The Metabase dashboard uses an additional view, `vw_adjusted_irs_with_position`, to support position filters and the hypothetical scenario tool. Because the multimodal workload dataset has no observed position field, anonymous athletes receive a stable demonstration position group for dashboard filtering; position-sensitive interpretations are therefore benchmark support, not actual athlete position observations.
...
Top injury types: Hamstring injury (1,256), Corona virus (986), Muscle injury (960), muscular problems (940), Knee injury (584). Season distribution is balanced: 1,750 bridge entries in 2020/21, up to 1,802 in 2024/25.
...
1. S1 has no player names or real dates, so it can only supply a workload time-series per anonymous athlete
2. S2 has named players with club affiliations that change across seasons, making it the only source that requires an M:N player-team bridge
3. S3 has no identifiers whatsoever and feeds only as an aggregate benchmark
...
The schema separates into three source-specific subject areas (S1 Multimodal, S2 European, S3 University) connected by two shared conformed dimensions.
...
\begin{figure}[p]
\centering
\includegraphics[width=0.78\paperheight,height=0.72\paperwidth,keepaspectratio]{docs/erd_v4.png}
\caption{Entity relationship diagram of the injury risk predictor schema.}
\label{fig-erd}
\end{figure}
\end{landscape}
\clearpage
...
The physical schema was implemented through DDL statements with the following key design decisions:
...
Composite indexes on `(mm_athlete_id, date_id)` are the primary optimization target since the rolling window functions partition on athlete and order by date. The measured impact of this index strategy is discussed in the Efficiency chapter.
...
A known consequence: athletes in `dim_athlete_multimodal` (S1) and players in `dim_player_european` (S2) are different populations that cannot be row-level matched. The adjusted IRS applies age-group-level context from S2 and S3 to individual S1 athletes. This is a benchmark comparison, not an individual-level linkage.
...
`INSERT IGNORE` respects the `UNIQUE KEY uq_eu_player_name`, so the same player appearing in multiple injury records is inserted only once.
...
The main analytical query computes, for each athlete and session-day, the acute load, chronic load, IRS, and risk band. It uses the following SQL constructs:
...
Together, they cover more than 20 distinct SQL keywords.
...
The table below summarizes the measured improvements. The `fact_injury_european` baseline uses the pre-optimization single-column index behavior; the after-plan uses the new composite `idx_inj_bridge_date`.
...
Additional execution-plan outputs are included in the Appendix as supplementary evidence.

## Database Cleanup
Once the transform pipeline completed, the temporary staging table `tmp_multimodal_prepared` was dropped. Removing it keeps the database schema clean and avoids storing redundant intermediate data.

# Visualization & Decision Support {#sec-Visualization}

Metabase is connected to the `injury_risk_predictor` database hosted on the VM. The project dashboard is published through a Metabase public sharing link for users on the HSLU network. For decision support, the dashboard is organized into three areas: a statistics overview, an observed athlete decision tool, and a hypothetical player scenario tool. This structure takes users from a population-level overview down to individual athlete inspection and, finally, scenario planning for players not yet in the system.

The Metabase configuration used for the dashboard is:

| Configuration Item | Value |
|---|---|
| Metabase host | `vm-fs26-019.dbm.ls.eee.intern:3000` |
| MySQL host from external clients | `vm-fs26-019.dbm.ls.eee.intern:3306` |
| MySQL host in Metabase configuration | `localhost:3306` |
| Database | `injury_risk_predictor` |
| Dashboard access | Metabase public dashboard link on the HSLU network |
| Dashboard URL | \href{http://vm-fs26-019.dbm.ls.eee.intern:3000/public/dashboard/1a79c6a5-de14-450d-b23a-b735c4335cf5}{\texttt{http://vm-fs26-019.dbm.ls.eee.intern:3000/}}<br>\href{http://vm-fs26-019.dbm.ls.eee.intern:3000/public/dashboard/1a79c6a5-de14-450d-b23a-b735c4335cf5}{\texttt{public/dashboard/1a79c6a5-de14-450d-b23a-b735c4335cf5}} |

![Metabase database connection settings for the MySQL database `injury_risk_predictor`. Metabase runs on the same VM as MySQL, so the connection host is configured as `localhost`.](Pictures/metabase_proof.jpg){#fig-metabase-proof width=95%}

For the PDF report, Metabase visuals are integrated as static screenshots. Direct live dashboard embedding is possible for an HTML report through Metabase public links or signed embeds, but a PDF submission needs reproducible image exports. The screenshot below was captured from the live public dashboard.

![Live Metabase public dashboard with filters, adjusted risk distribution, athlete table, observed athlete tool, and hypothetical scenario tool.](Pictures/metabase_live_public_dashboard_wide.png){#fig-metabase-live-dashboard width=88%}
...
Designing around three datasets with no shared player identifier forced a deliberate integration choice: conformed dimensions over a forced entity merge. This kept the schema honest, since no artificial joins are claimed, but it limits the analysis to aggregate-level cross-source comparisons. In hindsight, using `dim_age_group` as the sole cross-source key is methodologically sound for a proof-of-concept but would not scale to a production system where per-player longitudinal tracking is needed.
...
The performance measurements made the theoretical concepts concrete. The biggest takeaway was that indexes only help when the indexed columns actually match the query's filter pattern. In our case, adding a composite index on `risk_band` and `date_id` dropped the dashboard query from a full table scan at 266 ms to an index range scan at 17 ms. That is the kind of result that sticks.
...
The problem showed up most clearly in dependent tasks: the report, dashboard cards, reconciliation checks, and performance measurements all relied on the final database schema and view definitions. When it was unclear who would finish which part by when, things stalled and waiting became the default. Looking back, we would define responsibilities, realistic deadlines, dependencies, and expected outputs more explicitly at the beginning, so that each team member can plan their work better and delays can be identified earlier.
...
At the same time, the project showed that good teamwork requires regular coordination. Short check-ins and a shared task overview would have helped to keep everyone aligned when one person's work affected another part of the system. The main takeaway is that clearer task ownership, earlier communication when something slips, and more regular check-ins would have made the collaboration noticeably smoother.
...
The key lesson is that AI speeds up iteration but cannot replace validation against the actual system. Several outputs still had to be checked against the live database schema, row counts, Metabase results, and `EXPLAIN ANALYZE` plans before we could trust them. Going forward, we would use AI earlier for review checklists but keep a stricter evidence trail for every generated or revised technical claim.
...
The ELT and optimization pipeline runs in five reproducible steps:

1. DDL schema creation
2. Staging load
3. Dimension, fact, and view transformation
4. Optimization index creation
5. Materialized IRS refresh
