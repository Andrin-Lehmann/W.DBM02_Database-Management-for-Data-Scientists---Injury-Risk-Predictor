...
The current VM check confirms the intended optimization target. A dashboard-style filter on `fact_load_metrics` for selected risk bands and a date range still produces a table scan when `idx_lm_band_date` is absent:

```text
Filter: risk_band IN ('Caution','High Risk') AND date_id BETWEEN ...
-> Table scan on fact_load_metrics  (cost=1592 rows=15682)
```

The performance comparison is therefore reported as an implementation plan plus current status rather than as a fabricated benchmark:

| Approach | Current VM status | Expected effect |
|---|---|---|
| Baseline analytical query | Runs on 15,420 workload rows | Full window calculation over athlete histories |
| Composite index on `(mm_athlete_id, date_id)` | Defined in SQL, not yet visible in VM metadata check | Supports athlete/date ordering used by the rolling window |
| Materialized IRS table `fact_load_metrics` | Present with 15,420 rows | Lets Metabase read prepared IRS rows instead of recomputing ACWR |
| Covering index on `(risk_band, date_id)` | Defined in SQL, not yet visible in VM metadata check | Should reduce dashboard filter queries from table scan to indexed range lookup |
...
3. `sql/04_transform/51_create_indexes.sql` — indexing
3. `sql/05_transform/52_materialized_irs.sql` — materialized IRS table
