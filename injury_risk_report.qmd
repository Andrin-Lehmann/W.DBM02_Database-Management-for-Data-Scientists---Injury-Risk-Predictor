...
The index creation script defines this index idempotently through `create_index_if_missing`. It should be applied on the VM before collecting the final before/after `EXPLAIN ANALYZE` screenshots, because dropping and recreating indexes during report writing would modify the shared database state.
